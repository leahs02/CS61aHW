...
def pingpong(n):
    ...
    True
    """
    def total(n, i, current, next):
        if i == n:
            return current
        elif i % 8 == 0 or num_eights(i) >= 1:
            return total(n, i + 1, current - next, - next)
        else:
            return total(n, i + 1, current + next, next)
    return total(n, 1, 1, 1)
...
def count_coins(change):
    ...
    True
    """
    def helper(amount, current_smallest):
        if amount == 0:
            return 1
        if not current_smallest:
            return 0
        if current_smallest > amount:
            return 0
        with_current = helper(amount - current_smallest, current_smallest)
        without_current = helper(amount, next_larger_coin(current_smallest))
